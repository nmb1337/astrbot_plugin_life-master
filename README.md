...
- 🕐 **定时主动聊天**：机器人按设定的时间间隔主动发送消息，**同时支持群聊和私聊**
- 🎲 **双阶段智能生成**：先随机选定聊天意图（分享/关心/调侃/提问/吐槽/打招呼等），再生成自然消息
- 🚫 **黑名单过滤 + 自动重试**：自动拦截"我是AI""我没有历史记录"等机械回复，命中后自动重试
- 📋 **真实聊天记录注入**：将最近对话直接注入 Prompt，让 AI 自然延续话题而非"回忆聊天"
- 🎭 **8 种聊天意图**：分享日常、关心对方、调侃、提问、吐槽、分享见闻、延续话题、打招呼
- 🖥️ **可视化提示词管理**：内置 Plugin Page，支持增删改查提示词
- 🔇 **免打扰时段**：设定夜间等免打扰时段
- 🎨 **表情包适配**：若安装了「AI 智能表情包」插件，主动消息可自动搭配表情包
- 🎮 **管理指令**：提供 `/proactive_chat` 指令组
...
在插件配置中，填入需要机器人主动聊天的群号（**target_groups**）和私聊用户号（**private_targets**），每行一个：
...
| `private_targets` | list | [] | 目标私聊用户 QQ 号列表，每行一个 |
...
| `/proactive_chat trigger` | 管理员 | 立即在当前会话触发一次主动聊天（群聊和私聊均支持） |
...
aiocqhttp:PrivateMessage:111222333
```
